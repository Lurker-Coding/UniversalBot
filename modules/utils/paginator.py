...
class Pages:
    ...
    def __init__(self, ctx, *, entries, per_page=12, show_entry_count=True, hide_no_results=False):
        ...
        self.embed = discord.Embed(color=3553599)
    ...
    async def numbered_page(self):
        ...
        try:
            ...
            if page != 0 and page <= self.maximum_pages:
                ...
                to_delete.append(await self.channel.send(f'Invalid page given, please try again. ({page}/{self.maximum_pages})'))
                await asyncio.sleep(5)
...
class EmbedPages:
    ...
    async def show_page(self, page, *, first=False):
        ...
        p.append('')
        p.append('Confused? React with \N{INFORMATION SOURCE} for more info.')
        embed.description = '' if embed.description == discord.Embed.Empty else embed.description
        embed.description += '\n'.join(p)
